...
> ⚠️ 历史文档提示：本文档是当初的设计方案存档，后续演进以 `Readme.md` 和代码为准。主要变化：
> - 服务端已支持 UDP ASSOCIATE（QUIC/语音/DNS 可走隧道），新增 `udp_timeout` UCI 选项
> - `skip_private_check` 的判定从 `10.10.*` 改为标准 RFC1918 私网（10/8、172.16/12、192.168/16）
> - 认证回复、转发关闭等一批健壮性修复（见 main 分支提交记录）
